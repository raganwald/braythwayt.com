...
So with a bluebird you can chain functions together in series, while if you didn't have a bluebird all you could do is write functions that transform other functions. Not that there's anything wrong with that, we used that to great effect with [cardinals](http://braythwayt.com/homoiconic/2008-10-31/songs_of_the_cardinal.html#readme) and [quirky birds](http://braythwayt.com/homoiconic/2008-11-04/quirky_birds_and_meta_syntactic_programming.html#readme).
...
> This could be even more useful if it supported methods with blocks. Adventurous readers may want to combine this code with the tricks in [cardinal.rb](http://braythwayt.com/homoiconic/2008-10-31/cardinal.rb) and see if they can build a version of `before` that supports methods that take blocks.
...
One implementation meeting these requirements is here: [before\_and\_after\_advice.rb](http://braythwayt.com/homoiconic/2008-11-07/before_and_after_advice.rb "before_and_after_advice.rb"). Embedded in a lot of extra moving parts, the basic pattern of composing methods is still evident:
...
That is why we looked at supporting just before methods first. If you are comfortable with the [na&iuml;ve implementation of before advice](http://braythwayt.com/homoiconic/2008-11-07/naive_before_advice.rb) discussed above, the mechanism is easy to understand. The complete version is considerably more powerful. As mentioned, it supports before and after advice. It also uses `instance_exec` to evaluate the blocks in the receiver's scope, providing access to private methods and instance variables. And it works properly even when you override the method being advised.

Please give it a try and let me know what you think.

p.s. If the sample code gives an error, it could be [a known bug in Ruby 1.8](http://braythwayt.com/homoiconic/2008-11-09/proc_arity.html "Proc#arity"). Try declaring your advice with an empty parameter list, e.g. `do || ... end`.

p.p.s. [A comment on implementing method advice](http://braythwayt.com/homoiconic/2008-11-07/comment_on_implementing_advice.html#readme).
